...
class PrefixEvent extends VTEventHandler {
	...
	public function handleEvent($eventName, $data) {
		...
		$search=$adb->pquery("select eventhandler_id from vtiger_eventhandlers where event_name IN (?,?,?)", array('corebos.filter.ModuleSeqNumber.set','corebos.filter.ModuleSeqNumber.increment','corebos.filter.ModuleSeqNumber.get'));
		if($adb->num_rows($search) > 0){
